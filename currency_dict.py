CURRENCY_DISPLAY = {
    "004": ("AFA", "Afghani"),
    "008": ("ALL", "Lek"),
    "012": ("DZD", "Algerian Dinar"),
    "024": ("AON", "New Kwanza"),
    "031": ("AZM", "Azerbaijanian Manat"),
    "032": ("ARS", "Argentine Peso"),
    "036": ("AUD", "Australian Dollar"),
    "040": ("ATS", "Schilling"),
    "044": ("BSD", "Bahamian Dollar"),
    "048": ("BHD", "Bahraini Dinar"),
    "050": ("BDT", "Taka"),
    "051": ("AMD", "Armenian Dram"),
    "052": ("BBD", "Barbados Dollar"),
    "056": ("BEF", "Belgian Franc"),
    "060": ("BMD", "Bermudian Dollar"),
    "064": ("BTN", "Bhutan Ngultrum"),
    "068": ("BOB", "Boliviano"),
    "072": ("BWP", "Pula"),
    "076": ("BRC", "Cruzeiro"),
    "084": ("BZD", "Belize Dollar"),
    "090": ("SBD", "Solomon Islands Dollar"),
    "096": ("BND", "Brunei Dollar"),
    "100": ("BGL", "Lev"),
    "104": ("MMK", "Kyat"),
    "108": ("BIF", "Burundi Franc"),
    "116": ("KHR", "Riel"),
    "124": ("CAD", "Canadian Dollar"),
    "132": ("CVE", "Cape Verde Escudo"),
    "136": ("KYD", "Cayman Islands Dollar"),
    "144": ("LKR", "Sri Lanka Rupee"),
    "152": ("CLP", "Chilean Peso"),
    "156": ("CNY", "Yuan Renminbi"),
    "158": ("CNX", "Chinese Renminbi"),
    "170": ("COP", "Colombian Peso"),
    "174": ("KMF", "Comoro Franc"),
    "180": ("ZRN", "New Zaire"),
    "188": ("CRC", "Costa Rican Colon"),
    "191": ("HRK", "Kuna"),
    "192": ("CUP", "Cuban Peso"),
    "196": ("CYP", "Cyprus Pound"),
    "203": ("CZK", "Czech Koruna"),
    "208": ("DKK", "Danish Krone"),
    "214": ("DOP", "Dominican Peso"),
    "218": ("ECS", "Sucre"),
    "222": ("SVC", "El Salvador Colon"),
    "226": ("GQE", "Ekwele"),
    "230": ("ETB", "Ethiopian Birr"),
    "232": ("ERN", "Nakfa"),
    "233": ("EEK", "Kroon"),
    "238": ("FKP", "Pound"),
    "242": ("FJD", "Fiji Dollar"),
    "246": ("FIM", "Markka"),
    "250": ("FRF", "French Franc"),
    "262": ("DJF", "Djibouti Franc"),
    "270": ("GMD", "Dalasi"),
    "280": ("DM", "Deutsche mark"),
    "288": ("GHC", "Cedi"),
    "292": ("GIP", "Gibraltar Pound"),
    "300": ("GRD", "Drachma"),
    "320": ("GTQ", "Quetzal"),
    "324": ("GNF", "Guinea Franc"),
    "328": ("GYD", "Guyana Dollar"),
    "332": ("HTG", "Gourde"),
    "340": ("HNL", "Lempira"),
    "344": ("HKD", "Hong Kong Dollar"),
    "348": ("HUF", "Forint"),
    "352": ("ISK", "Iceland Krona"),
    "356": ("INR", "Indian Rupee"),
    "360": ("IDR", "Rupiah"),
    "364": ("IRR", "Iranian Rial"),
    "368": ("IQD", "Iraqi Dinar"),
    "372": ("IEP", "Irish Pound"),
    "376": ("ILS", "New Israeli Sheqel*"),
    "380": ("ITL", "Italian Lira"),
    "388": ("JMD", "Jamaican Dollar"),
    "392": ("JPY", "Yen"),
    "398": ("KZT", "Tenge"),
    "400": ("JOD", "Jordanian Dinar"),
    "404": ("KES", "Kenyan Shilling"),
    "408": ("KPW", "North Korean Won"),
    "410": ("KRW", "Won"),
    "414": ("KWD", "Kuwaiti Dinar"),
    "417": ("KGS", "Som"),
    "418": ("LAK", "Kip"),
    "422": ("LBP", "Lebanese Pound"),
    "426": ("LSL", "Lesotho Loti"),
    "428": ("LVL", "Latvian Lats"),
    "430": ("LRD", "Liberian Dollar"),
    "434": ("LYD", "Libyan Dinar"),
    "440": ("LTL", "Lithuanian Litas"),
    "442": ("LUF", "Luxembourg Franc"),
    "446": ("MOP", "Pataca"),
    "450": ("MGF", "Malagasy Franc"),
    "454": ("MWK", "Kwacha"),
    "458": ("MYR", "Malaysian Ringgit"),
    "462": ("MVR", "Rufiyaa"),
    "466": ("MLF", "Mali"),
    "470": ("MTL", "Maltese Lira"),
    "478": ("MRO", "Ouguiya"),
    "480": ("MUR", "Mauritius Rupee"),
    "484": ("MXN", "Mexican Peso"),
    "496": ("MNT", "Tugrik"),
    "498": ("MDL", "Moldovan Leu"),
    "504": ("MAD", "Moroccan Dirham"),
    "508": ("MZM", "Metical"),
    "512": ("OMR", "Rial Omani"),
    "516": ("NAD", "Naminian Dollar"),
    "524": ("NPR", "Nepalese Rupee"),
    "528": ("NLG", "Netherlands Guilder"),
    "532": ("ANG", "Netherlands"),
    "533": ("AWG", "Aruban Guilder"),
    "548": ("VUV", "Vatu"),
    "554": ("NZD", "New Zealand Dollar"),
    "558": ("NIO", "Cordoba Oro"),
    "566": ("NGN", "Naira"),
    "578": ("NOK", "Norwegian Krone"),
    "586": ("PKR", "Pakistan Rupee"),
    "590": ("PAB", "Balboa"),
    "598": ("PGK", "Kina"),
    "600": ("PYG", "Guarani"),
    "604": ("PEN", "Nuevo Sol"),
    "608": ("PHP", "Philippine Peso"),
    "620": ("PTE", "Portuguese Escudo"),
    "624": ("GWP", "Guinea-Bissau Peso"),
    "626": ("TPE", "Timor Escudo"),
    "634": ("QAR", "Qatari Rial"),
    "642": ("ROL", "Leu"),
    "643": ("RUB", "Russian rouble (643)"),
    "646": ("RWF", "Rwanda Franc"),
    "654": ("SHP", "St Helena Pound"),
    "678": ("STD", "Dobra"),
    "682": ("SAR", "Saudi Riyal"),
    "690": ("SCR", "Seychelles Rupee"),
    "694": ("SLL", "Leone"),
    "702": ("SGD", "Singapore Dollar"),
    "703": ("SKK", "Slovak Koruna"),
    "704": ("VND", "Dong"),
    "705": ("SIT", "Tolar"),
    "706": ("SOS", "Somali Shilling"),
    "710": ("ZAR", "Rand"),
    "716": ("ZWD", "Zimbabwe Dollar"),
    "720": ("YDD", "Yemeni Dinar"),
    "724": ("ESP", "Spanish Peseta"),
    "736": ("SDD", "Sudanese Dinar"),
    "740": ("SRG", "Surinam Guilder"),
    "748": ("SZL", "Lilangeni"),
    "752": ("SEK", "Swedish Krona"),
    "756": ("CHF", "Swiss Franc"),
    "760": ("SYP", "Syrian Pound"),
    "762": ("TJR", "Tajik Ruble"),
    "764": ("THB", "Baht"),
    "776": ("TOP", "Tongan Pa'anga"),
    "780": ("TTD", "Trinidad and Tobago Dollar"),
    "784": ("AED", "UAE Dirham"),
    "788": ("TND", "Tunisian Dinar"),
    "792": ("TRL", "Turkish Lira"),
    "793": ("PTL", "Pseudo Turkish Lira"),
    "795": ("TMM", "Manat"),
    "800": ("UGX", "Uganda Shilling ++"),
    "807": ("MKD", "Denar"),
    "810": ("???", "Russian rouble"),
    "818": ("EGP", "Egyptian Pound"),
    "826": ("GBP", "Pound sterling"),
    "834": ("TZS", "Tanzanian Shilling"),
    "840": ("USD", "US dollar"),
    "858": ("UYU", "Peso Uruguayo"),
    "860": ("UZS", "Uzbekistan Sum"),
    "862": ("VEB", "Bolivar"),
    "882": ("WST", "Tala"),
    "886": ("YER", "Yemeni Rial"),
    "890": ("YUN", "New Yugoslavian Doll"),
    "891": ("YUM", "New Dinar"),
    "894": ("ZMK", "Kwacha"),
    "901": ("TWD", "New Taiwan Dollar"),
    "906": ("PYB", "Belarus rouble"),
    "924": ("ZWG", "Zimbabwe Gold"),
    "925": ("SLE", "Leone (NEW)"),
    "926": ("VED", "Bolivar Soberano"),
    "928": ("VES", "Bolivar Soberano (NEW)"),
    "929": ("MRU", "Ouguiya (NEW)"),
    "930": ("STN", "Dobra (NEW)"),
    "931": ("CUC", "Peso Convertible"),
    "932": ("ZWL", "Zimbabwe Dollar (NEW)"),
    "933": ("BYN", "Belarusian Ruble"),
    "934": ("TMT", "Turkmenistan New Manat"),
    "936": ("GHS", "Ghana cedi"),
    "937": ("VEF", "Bolivar Fuerte"),
    "938": ("SDG", "Sudanese Pound"),
    "940": ("UYI", "Uruguay Peso en Unidades Indexadas (UI)"),
    "941": ("RSD", "Serbian Dinar"),
    "943": ("MZN", "Mozambique Metical"),
    "944": ("AZN", "Azerbaijanian Manat (NEW)"),
    "946": ("RON", "Romanian Leu (NEW)"),
    "947": ("CHE", "WIR Euro"),
    "948": ("CHW", "WIR Franc"),
    "949": ("TRY", "New Turkish Lira"),
    "950": ("XAF", "CFA Franc BEAC"),
    "951": ("XCD", "East Caribbean Dollar"),
    "952": ("XOF", "CFA Franc BCEAO"),
    "953": ("XPF", "CFP Franc"),
    "967": ("ZMW", "Zambian Kwacha"),
    "968": ("SRD", "New Suriname dollar"),
    "969": ("MGA", "Malagasy ariary (Madagascar)"),
    "970": ("COU", "Unidad de Valor Real"),
    "971": ("AFN", "Afghanistan Afghani (NEW)"),
    "972": ("TJS", "Tajikistan Somoni"),
    "973": ("AOA", "Kwanza (NEW)"),
    "974": ("PYB", "Belarus rouble"),
    "975": ("BGN", "Bulgarian Lev"),
    "976": ("CDF", "Franc Congolais"),
    "977": ("BAM", "Convertible Marks"),
    "978": ("EUR", "EURO"),
    "979": ("MXV", "Mexican Unidad de Inversion (UDI)"),
    "980": ("UAH", "Hryvnia"),
    "981": ("GEL", "Lari"),
    "984": ("BOV", "Mvdol"),
    "985": ("PLN", "Zloty"),
    "986": ("BRL", "Brazilian Real"),
}